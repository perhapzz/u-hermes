...
如果 U 盘是从仓库克隆来的「裸版本」（只有源码，没有 `app/` 目录），需要先跑一次 setup：

```text
Windows:   双击  setup.bat
macOS:     在终端运行  bash setup.sh
```

setup 会自动下载独立 Python 解释器和所有依赖到 U 盘的 `portable/app/` 目录。整个过程需要联网，国内会自动用清华和华为云镜像。完成后不需要再跑第二次。
...
启动后访问 <http://127.0.0.1:8787/> 就能看到 Hermes 的 Web UI。关闭命令行窗口即可停止服务。
...
U 盘根目录的结构如下。你直接接触的是最外层的 4 个启动脚本，其它都是程序内部细节。

```text
u-hermes/                       <-- U 盘根目录
├── setup.bat / setup.sh         一键搭建（首次使用）
...
### 3.2 Windows 需要 Git for Windows

Hermes 的 shell 工具依赖 bash 来执行命令。在 Windows 上，需要预先装好 [Git for Windows](https://git-scm.com/download/win)。启动脚本会自动找到它，不需要额外配置。

> **为什么不用 WSL？** Windows 自带的 `bash.exe` 实际上是 WSL 的启动器。WSL 配置复杂、容易因代理问题坏掉。Git for Windows 自带的 Git Bash 简单稳定，是推荐方案。

### 3.3 跨平台 U 盘

如果你希望同一只 U 盘既能在 Mac 用又能在 Windows 用，在 Mac 上跑：

```bash
bash setup.sh --all-platforms
```

这会同时下载 Mac ARM64 / Mac Intel / Windows x64 三个版本的 Python 运行时。代价是 U 盘多占 ~600MB 空间。
...
说明启动器找到了系统的 WSL 启动器而不是 Git Bash。已在 v0.1 修复，现在脚本会优先使用 Git Bash 并拒绝 WSL stub。如果遇到，确保装了 Git for Windows，然后删掉 `portable/app/packages/` 重装。

### 5.4 setup 下载 Python 很慢

setup 检测到国内网络会自动用清华源 + ghfast.top + 华为云镜像。如果还是慢，可以手动下载 standalone Python 解压到 `portable/app/runtime/`。
...
这个仓库本身就是 U 盘的内容（除了大依赖）。git 里的 `portable/` 目录就是 U 盘上的 `portable/`。`setup.sh` / `setup.bat` 只是把太大、不适合放 git 的东西（Python 解释器 + pip 包）下载进来。**所以你看到的项目结构 = 用户看到的 U 盘结构。**

---

*本文档使用 Markdown 编写，通过 Edge 浏览器的打印功能转换为 PDF。重新生成请运行 `docs/build_pdf.ps1`。*
