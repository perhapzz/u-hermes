...
macOS:     双击  Mac-Start.command（或在终端运行  bash Mac-Start.command）
```
...
```text
u-hermes/                       <-- U 盘根目录
├── Windows-Start.bat                    Windows 双击启动（首次自动装依赖）
├── Mac-Start.command                macOS 双击启动（首次自动装依赖）
├── README.md                    项目说明
└── portable/                    程序内部（不需要进入）
    ├── webui/                   Web UI 源码
    ├── agent/                   AI Agent 源码
...
说明启动器找到了系统的 WSL 启动器而不是 Git Bash。已在 v0.1 修复。现在 `Windows-Start.bat` 优先使用 U 盘自带的 PortableGit，不会再踩这个坑。如果还遇到，删掉 `portable/app/packages/` 重新启动。
...
这个仓库本身就是 U 盘的内容（除了大依赖）。git 里的 `portable/` 目录就是 U 盘上的 `portable/`。`Windows-Start.bat` / `Mac-Start.command` 只是把太大、不适合放 git 的东西（Python 解释器 + PortableGit + pip 包）下载进来。**所以你看到的项目结构 = 用户看到的 U 盘结构。**
